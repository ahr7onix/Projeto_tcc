...
import { Card, PageHeader, ChipGroup, EmptyState, Btn, Input, Textarea, Select } from '../../components/ui'
...
export default function RegistrosPage() {
  ...
  const handleSaveGlicemia = async () => {
    if (!gValor) return
    setGSaving(true)
    setTimeout(() => { setGSaving(false); setTab('lista'); setGValor(''); setGObs('') }, 800)
  }

  const handleSaveRefeicao = async () => {
    if (!rDesc) return
    setRSaving(true)
    setTimeout(() => { setRSaving(false); setTab('lista'); setRDesc(''); setRCarb(''); setRObs('') }, 800)
  }
      ...
      {tab === 'lista' && (
        <>
          ...
          <div style={{ display: 'flex', gap: 16, marginBottom: 24, flexWrap: 'wrap' }}>
            {[
              { label: 'Registros glicemia · 7d', value: '0', bg: 'var(--primary-soft)', fg: 'var(--primary)', icon: <DropIcon /> },
              { label: 'Refeições · 7d', value: '0', bg: 'var(--success-soft)', fg: 'var(--success)', icon: <ForkIcon /> },
            ].map((s) => (

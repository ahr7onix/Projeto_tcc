...
export default function RegistrosPage() {
  ...
  const handleSaveGlicemia = async () => {
    if (!gValor) return
    setGSaving(true); setError(null)
    try {
      await api.post('/registros/glicemia', { valor: Number(gValor), momento: gMomento, observacao: gObs })
      setTab('lista'); setGValor(''); setGObs(''); setGMomento('jejum')
    } catch (err) {
      setError(extractError(err))
  ...
  const handleSaveRefeicao = async () => {
    if (!rDesc) return
    setRSaving(true); setError(null)
    try {
      await api.post('/registros/refeicao', {
        descricao: rDesc, tipo_refeicao: rTipo,
        carboidratos: rCarb ? Number(rCarb) : undefined, observacao: rObs,
      })
      setTab('lista'); setRDesc(''); setRCarb(''); setRObs('')
    } catch (err) {
      setError(extractError(err))
        ...
        <>
          ...
          <Card title="Linha do tempo">
            ...
            {loading ? (
              ...
              <EmptyState icon={<HourglassIcon />} title="Sem registros ainda" message="Comece registrando uma medição de glicemia ou uma refeição." />
            ) : (
              <div>
                {registros.map((r, i) => (
                  <div key={r.id} style={{
        ...
        <div style={{ maxWidth: 560 }}>
          <Card title="Nova medição de glicemia">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <Input label="Valor (mg/dL)" type="number" placeholder="Ex.: 110" value={gValor} onChange={(e) => setGValor(e.target.value)} />
              <div>
              ...
              <Textarea label="Observações" placeholder="Sintomas, atividade física, medicação..." value={gObs} onChange={(e) => setGObs(e.target.value)} />
      ...
      {tab === 'nova-refeicao' && (
        <div style={{ maxWidth: 560 }}>
          <Card title="Nova refeição">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <Select label="Tipo de refeição" value={rTipo} onChange={(e) => setRTipo(e.target.value)} options={tipoRefeicaoOpts} />
              <Textarea label="O que foi consumido?" placeholder="Ex.: arroz, feijão, frango grelhado, salada" value={rDesc} onChange={(e) => setRDesc(e.target.value)} />
              <Input label="Carboidratos estimados (g) — opcional" type="number" placeholder="Quantidade estimada" value={rCarb} onChange={(e) => setRCarb(e.target.value)} />
              <Textarea label="Observações" placeholder="Como o paciente se sentiu, dificuldades..." value={rObs} onChange={(e) => setRObs(e.target.value)} />
